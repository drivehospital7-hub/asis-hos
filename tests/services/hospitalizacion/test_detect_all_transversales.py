    ...
    def test_engine_path_structure(self):
        with patch("app.database.get_session") as m_gs:
            with patch("app.services.engine.rule_based_detector.RuleBasedDetector") as m_dc:
                ...
                mod = __import__("app.services.hospitalizacion.detect_all", fromlist=["detect_all_problems_hospitalizacion"])
                func = getattr(mod, "detect_all_problems_hospitalizacion")
                wb, idx = _build_sheet()
                restore = _env("true")
                try:
