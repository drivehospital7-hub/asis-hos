    ...
    def test_engine_path_structure(self):
        with patch("app.database.get_session") as m_gs:
            with patch("app.services.engine.rule_based_detector.RuleBasedDetector") as m_dc:
                ...
                mod = __import__("app.services.intramural.detect_all", fromlist=["detect_all_problems_intramural"])
                func = getattr(mod, "detect_all_problems_intramural")
                wb, idx = _build_sheet()
                restore = _env("true")
                try:
