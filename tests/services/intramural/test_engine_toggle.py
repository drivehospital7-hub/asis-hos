"""Strict TDD: Tests for engine toggle in intramural/detect_all.py.
...
class TestIntramuralEngineToggle:
    """Tests for the engine toggle in detect_all_problems_intramural."""
    ...
    def test_engine_path_evaluates_resolver_rules_only(
        ...
        served = [
            ...
            _regla("nueva_regla_ui", "intramural", "Decimales"),
        ]
        fake_resolver = MagicMock()
        fake_resolver.resolve.return_value = served
        _RecordingDetector.reset({
            "bacteriologas_cronograma": [{"factura": "FAC-001", "problema": "crono"}],
        })
        ...
        with patch(
            ...
            result, responsables = detect_all_problems_intramural(
                wb.active, indices,
            )

        assert set(_RecordingDetector.instances) == {
            "valores_decimales", "bacteriologas_cronograma", "nueva_regla_ui",
        }
        assert "problemas" in result
        assert isinstance(result["problemas"], dict)
        assert "totales" in result
        assert result["area"] == AREA_INTRAMURAL
        assert len(result["problemas"]["profesionales"]) == 1
        assert responsables == {}

    @patch("app.services.intramural.detect_all.is_rule_engine_enabled", return_value=False)
    ...
    def test_engine_path_with_all_detectors(
        ...
        expected_keys = {
            ...
            "profesionales", "duplicado_id_codigo", "revision_cantidad",
        }
