"""ODO parity harness (Slice 2, strict TDD).

Unified /procesar (process_unified) must match the legacy odontologia
...
LOPEZ = "Lopez Andrade Claudia Lorena"

ODO_REQUIRED_HEADERS: dict[str, str] = {
...
def _build_odo_workbook() -> Workbook:
    """Minimal ODO fixture with exact headers."""
    wb = Workbook()
    ws = wb.active
    headers = list(ODO_REQUIRED_HEADERS.values())
    for col, name in enumerate(headers, start=1):
        ws.cell(row=1, column=col, value=name)
    rows = [
        ("FAC-ODO-001", "PAC-001", CONVENIO_PYP, "890203", "Consultas", 1, 1000, 2000),
        ("FAC-ODO-002", "PAC-001", CONVENIO_PYP, "997002", "Consultas", 2, 1000, 2000),
        ("FAC-ODO-003", "PAC-002", "Asistencial", "997106", "Preventivo", 1, 1500.75, 2000),
        ("FAC-ODO-004", "PAC-003", CONVENIO_PYP, "997301", "Preventivo", 12, 1000, 2000),
    ]
    for r, (fact, pac, conv, cod, tipo, cant, vs, vp) in enumerate(rows, start=2):
        ...
        ws.cell(row=r, column=10, value=LOPEZ)
        ws.cell(row=r, column=11, value="ODONTOLOGIA")
...
def _indices(wb: Workbook) -> dict[str, int | None]:
    ...
    indices, _ = get_column_indices(headers, ODO_REQUIRED_HEADERS)
    return indices
...
class TestOdoParity:
    def test_unified_matches_legacy_order_insensitive(self) -> None:
        ...
        wb = _build_odo_workbook()
        indices = _indices(wb)
        with (
            patch("app.services.odontologia.detect_all.is_rule_engine_enabled", return_value=False),
            ...
            legacy, _ = detect_all_problems_odontologia_por_responsable(wb.active, indices)
            unified, _ = process_unified(wb.active, indices)
        assert unified["area"] == "unificada"
        assert legacy["area"] == "odontologia"
    ...
    def test_engine_mocked_ruta_exception_parity(self) -> None:
        """Engine ON (mocked RuleBasedDetector): 990203 exemption matches legacy."""
        from app.services.odontologia.detect_por_responsable import (
            detect_all_problems_odontologia_por_responsable,
        )
        from app.services.unified_processor import process_unified
        ...
        indices = _indices(wb)
        payload = [
            {"identificacion": "PAC-001", "factura": "FAC-ODO-001", "cantidad": 3},
        ...
        with (
            ...
            patch("app.services.odontologia.detect_all.RuleBasedDetector", create=True) as _alias,
        ):
            mock_session_mgr.return_value.__enter__.return_value = MagicMock()
            mock_detector_cls.side_effect = _mock_detector
            _alias.side_effect = _mock_detector
            with patch("app.database.get_session", return_value=MagicMock()):
                legacy, _ = detect_all_problems_odontologia_por_responsable(wb.active, indices)
                unified, _ = process_unified(wb.active, indices)
...
class TestProcesarRouteContract:
    def test_procesar_route_stays_delegator(self) -> None:
        """POST /procesar must delegate; zero Polars/openpyxl/detection logic."""
        text = Path("app/routes/procesar.py").read_text(encoding="utf-8")
        assert "detect_problems_only" in text
        assert "import polars" not in text
        assert "openpyxl" not in text
        assert "def detect_" not in text
        assert '"status"' in text and '"data"' in text and '"errors"' in text
