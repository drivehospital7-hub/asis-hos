...
interface OdontologiaData {
  can_write: boolean;
  username: string;
  profesionales: Array<{ codigo: string; nombre: string; identificacion: string }>;
}

const initialData = (window as unknown as { __INITIAL_DATA__?: OdontologiaData }).__INITIAL_DATA__;
...
export function OdontologiaPage() {
  ...
  const profesionales = initialData?.profesionales ?? [];
  ...
  const handleProcesar = async () => {
    ...
    try {
      ...
      if (profesional) fd.append("profesional", profesional);
      const res = await fetch("/odontologia/", { method: "POST", body: fd });
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <Breadcrumbs items={[{ label: "Odontología" }]} />
      <PageTitle
        eyebrow="Odontología"
        title="Consumos y Servicios"
        description="Carga el reporte detallado de odontología en formato Excel para validar los registros y detectar inconsistencias."
      />
      ...
      <Card className="p-6 border-border bg-card shadow-none mb-6">
        ...
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label htmlFor="profesional" className="block text-xs font-semibold text-foreground mb-1.5 uppercase tracking-wider">
              Profesional
            </label>
            <select
              id="profesional"
        ...
        <div className="mt-4 flex justify-between items-center">
          ...
          >
            ...
            {showRules ? "Ocultar reglas" : "Listar reglas"}
          </Button>
        </div>

        {showRules && (
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-4">
            <h3 className="font-display font-semibold text-foreground mb-3">Reglas de Validación - Odontología</h3>
            <ul className="space-y-2 text-sm text-foreground/80">
              <li><strong>1. Decimales:</strong> Detecta valores con decimales en columnas de cantidad</li>
              ...
              <li><strong>4. Convenio - Procedimiento:</strong> El procedimiento no corresponde al convenio</li>
              <li><strong>5. Cantidades anómalas:</strong> Valores que parecen incorrectos</li>
              <li><strong>6. Tipo Identificación:</strong> El tipo de documento no coincide con la edad</li>
              <li><strong>7. Centro de costo:</strong> Validación de centro de costo</li>
              <li><strong>8. Entidad Cobrar vs Afiliación:</strong> Compara el código de entidad</li>
            </ul>
          </div>
        )}
