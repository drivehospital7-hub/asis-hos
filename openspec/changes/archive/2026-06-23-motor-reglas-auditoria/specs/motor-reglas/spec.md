...
### R5: Versioning and State Machine

Rules MUST have a version (integer, auto-incremented on modification). States SHALL follow: draft → active → deprecated → retired. Only `active` rules are evaluated by default. `deprecated` rules MAY be evaluated when overridden by an active exception. Modification to an active rule creates a new version; the previous version is archived.
