"""Orquestador de detección de problemas para Ambulatoria.

Agrupa detectores transversales + específicos de Ambulatoria.
"""
...
def _get_ambulatoria_detectors() -> list[Callable]:
    """Returns list of Ambulatoria-specific detector callables.
    
    Used by tipo_factura_registry for lazy import.
    """
    from app.services.ambulatoria.centro_costo_ambulatoria import (
        detect_centro_costo_ambulatoria,
    )
    return [
        detect_centro_costo_ambulatoria,
    ]
...
def detect_all_problems_ambulatoria(
    data_sheet: Worksheet,
    indices: dict[str, int | None],
) -> tuple[dict[str, Any], dict[str, str]]:
    """Detecta TODOS los problemas en facturas de Ambulatoria.
    ...
    # 1. Centro Costo
    problemas_centros = detect_centro_costo_ambulatoria(data_sheet, indices)
