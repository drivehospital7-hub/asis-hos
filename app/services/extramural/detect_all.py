...
def _get_extramural_detectors() -> list[Callable]:
    """Returns list of Extramural-specific detector callables.
...
def detect_all_problems_extramural(
    ...
    if is_rule_engine_enabled():
        ...
        session = get_session()
        try:
            decimales = RuleBasedDetector("valores_decimales", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            if _PERSIST:
                session.commit()
            else:
                session.rollback()
    ...
    if is_rule_engine_enabled():
        ...
        session = get_session()
        try:
            r1 = RuleBasedDetector("tipo_documento_edad_menor_7", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            r2 = RuleBasedDetector("tipo_documento_edad_mayor_18", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            r3 = RuleBasedDetector("tipo_documento_edad_7_17", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            r4 = RuleBasedDetector("tipo_documento_edad_as_menor", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            r5 = RuleBasedDetector("tipo_documento_edad_ms_mayor", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            r6 = RuleBasedDetector("tipo_documento_edad_cn_invalido", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            r7 = RuleBasedDetector("tipo_documento_edad_ce_invalido", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
    ...
    if is_rule_engine_enabled():
        ...
        session = get_session()
        try:
            r1 = RuleBasedDetector("tipo_id_requiere_entidad_86000", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            r2 = RuleBasedDetector("entidad_86000_requiere_as_ms", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
    ...
    if is_rule_engine_enabled():
        ...
        session = get_session()
        try:
            entidad_afiliacion_comparison = RuleBasedDetector("codigo_entidad", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            if _PERSIST:
                session.commit()
            else:
                session.rollback()
    ...
    if is_rule_engine_enabled():
        ...
        session = get_session()
        try:
            tipo_usuario = RuleBasedDetector("tipo_usuario_valido", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            if _PERSIST:
                session.commit()
            else:
                session.rollback()
    ...
    if is_rule_engine_enabled():
        ...
        session = get_session()
        try:
            copago_entidad = RuleBasedDetector("copago_entidad_valido", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            if _PERSIST:
                session.commit()
            else:
                session.rollback()
    ...
    if is_rule_engine_enabled():
        ...
        session = get_session()
        try:
            cups_sin_contrato = RuleBasedDetector("cups_sin_contrato", session, dominio=AREA_EXTRAMURAL).detect(data_sheet, indices, persist=_PERSIST)
            if _PERSIST:
                session.commit()
            else:
                session.rollback()
