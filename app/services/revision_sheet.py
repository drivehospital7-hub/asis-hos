...
def _detect_centro_costo_urgencias(
    ...
    -Regla 8: Código en (890601H, 39133) Y Centro != HOSPITALIZACIÓN - ESTANCIA GENERAL
    -Regla 9: Tarifario="Suministros, Medicamentos" Y Centro != APOYO TERAPEUTICO-FARMACIA E INSUMOS.
    ...
    logger.warning("Índices relevantes - codigo_tipo_proc: %s, codigo: %s, codigo_equiv: %s, laboratorio: %s, centro_costo: %s, ide_contrato: %s, codigo_entidad: %s, tipo_factura: %s, tarifario: %s",
                codigo_tipo_proc_idx, codigo_idx, codigo_equiv_idx, laboratorio_idx, centro_costo_idx, ide_contrato_idx, codigo_entidad_cobrar_idx, tipo_factura_descripcion_idx, tarifario_idx)
    ...
    for row in range(2, data_sheet.max_row + 1):
        ...
        # Leer tarifario de ESTA fila (siempre, para todas las filas)
    ...
    for factura_str, data in factura_sala_data.items():
        ...
        if codigos_sala:
            ...
            # Solo aplica si los activadores son no-SOAT (5DSB01 / 05DSB01 / 129B02)
            # Si los activadores son SOAT (38114 / 38915), los obligatorios son otros (39145 / 39131)
    ...
    for row in range(2, data_sheet.max_row + 1):
        ...
        if row <= 5:
            logger.info("Fila %s DEBUG: factura=%s, codigo_tipo_proc=%s, codigo=%s, tipo_factura=%s, laboratorio=%s, centro_costo=%s, ide_contrato=%s, tarifario=%s",
                       row, factura_str, repr(codigo_tipo_proc), repr(codigo), repr(tipo_factura_descripcion), repr(laboratorio), repr(centro_costo), repr(ide_contrato), repr(tarifario))
        ...
        # ----- Regla 9: Tarifario="Suministros, Medicamentos" -> Centro debe ser "APOYO TERAPEUTICO-FARMACIA E INSUMOS."
        ...
        # ----- Regla 9 REVERSE: Centro=FARMACIA -> Tarifario debe ser "Suministros, Medicamentos"
        if centro_costo_str == CENTRO_COSTO_FARMACIA:
            if tarifario_str != VALOR_TARIFARIO_FARMACIA:
                problemas_centros.append({
                    "factura": factura_str,
        ...
        # ----- Regla 8: Código en {890601H, 39133} -> Centro de costo debe ser "HOSPITALIZACIÓN - ESTANCIA GENERAL"
...
# ----- Nueva Regla: Tipo Factura=Hospitalización + Centro Costo=URGENCIAS -> Error (debe ser "HOSPITALIZACIÓN - ESTANCIA GENERAL")
        ...
        if codigo_entidad_str == ENTIDAD_IDE_CONTRATO_ESS118_NUEVOS and codigo_excluir in (
            CODIGO_IDE_CONTRATO_735301_ESS118,
            CODIGO_IDE_CONTRATO_861801_ESS118,
        ):
            if ide_contrato_str not in IDE_CONTRATO_MULTIPLE_ESS118_NUEVOS:
                problemas_ide_contrato.append({
                    ...
                    "nota": "ESS118 + 735301/861801 -> IDE 970 o 974",
                })

        # ----- ESS118 + Código=890405 -> IDE Contrato debe ser 974
        if codigo_entidad_str == ENTIDAD_IDE_CONTRATO_890405_ESS118 and codigo_excluir == CODIGO_IDE_CONTRATO_890405_ESS118:
            if ide_contrato_str != IDE_CONTRATO_REQUERIDO_890405_ESS118:
                problemas_ide_contrato.append({
                    "factura": factura_str,
                    "procedimiento": proc_str,
                    "codigo": codigo_excluir,
                    "entidad": codigo_entidad_str,
                    "ide_contrato_actual": ide_contrato_str,
                    "ide_contrato_deberia": IDE_CONTRATO_REQUERIDO_890405_ESS118,
                })

        # ----- ESS118 + Código=890205 -> IDE Contrato debe ser 970
        if codigo_entidad_str == ENTIDAD_IDE_CONTRATO_890205_ESS118 and codigo_excluir == CODIGO_IDE_CONTRATO_890205_ESS118:
            if ide_contrato_str != IDE_CONTRATO_REQUERIDO_890205_ESS118:
                problemas_ide_contrato.append({
                    "factura": factura_str,
                    "procedimiento": proc_str,
                    "codigo": codigo_excluir,
                    "entidad": codigo_entidad_str,
                    "ide_contrato_actual": ide_contrato_str,
                    "ide_contrato_deberia": IDE_CONTRATO_REQUERIDO_890205_ESS118,
                })
